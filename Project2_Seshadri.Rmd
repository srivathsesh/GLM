...
While the peanalized logistic regression model is used for feature selection, it may be used for prediction as well. The ROC curves for the training and test samples are shown below.
...
predict.test.class <- ifelse(glmnet.test >= 0.5 ,1, 0) # Did not use Optimal cut off
...
miss.class.train <- 1 - sum(diag(confusionMatrix.train))/sum(confusionMatrix.train)
