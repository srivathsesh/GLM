...
dffeatures <- as.matrix(unique(c(Pred.DecisionTrees,Pred.Lasso))[-14],ncol = 3)
#dffeatures <- dffeatures[which(dffeatures == '"(Intercept)"'*-1)]
# dffeatures <- dffeatures [-14,]
# dffeatures <- matrix(dffeatures, ncol = 3)
knitr::kable(dffeatures[-14,1], caption = "Selected Features")
```
...
training.logistic <- training[complete.cases(training),c(dffeatures,"TARGET_FLAG",jobsvars)]
write.csv(training.logistic, file = 'traininglogistic.csv')
test.logistic <- test[complete.cases(test),c(dffeatures,"TARGET_FLAG",jobsvars)]
train.logistic.full <- glm(TARGET_FLAG ~ . , family = binomial,data = training.logistic)
```

A logistic regression with the above selected predcitors (section 3.4) is fitted. The summary of the fit is shown below. It can
be seen from the Chi-squared goodness of fit that the model is adequate. There are some predictors whose slope is not significantly different from 0. 
...
paste("Goodness of fit test... P-Value" ,round(1 - pchisq(deviance(train.logistic.full),nrow(training.logistic) - length(train.logistic.full$coefficients)),2))
...
The performance of the model on complete cases seem to be good. However, the model does require features that has data missing. We'll use the KNN imputation method to impute data. 
...
plot(rocCurve.Imp.test.logistic, asp = NA, legacy.axes = T)
pROC::auc(rocCurve.Imp.test.logistic)
```
